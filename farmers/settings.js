...
function getFarmerDisplayName(user) {
    return user.displayName || (user.email ? user.email.split("@")[0] : "Farmer");
}
...
const IMGBB_API_KEY = "e04d8f002c165fc91a0299f6acf3f938";
...
function init() {
    ...
    auth.onAuthStateChanged(async (user) => {
        ...
        if (signedInAsEl) signedInAsEl.textContent = escapeHtml(getFarmerDisplayName(user));
