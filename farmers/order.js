...
function renderOrdersList(containerEl, orders) {
    ...
    orders.forEach((order) => {
        ...
        const buyerName = order.buyerName || order.buyerEmail || "Buyer";
        const totalPrice = formatNaira(order.totalPrice);
        const dateStr = formatDate(order.createdAt);
        const chatHref = order.buyerId
            ? `messages.html?buyerId=${encodeURIComponent(order.buyerId)}&buyerName=${encodeURIComponent(buyerName)}`
            : "messages.html";
      ...
      <article class="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 hover:bg-slate-50/80 transition gap-4">
        <div class="flex items-center gap-3 min-w-0">
          ...
          <div class="min-w-0">
            ...
            <p class="text-[11px] text-slate-500 truncate">${escapeHtml(productName)} for ${escapeHtml(buyerName)}</p>
        ...
        <div class="flex items-center justify-between sm:justify-end gap-4 shrink-0">
          <a href="${chatHref}" class="text-[10px] font-bold text-emerald-700 hover:text-emerald-800 hover:underline">
            Chat
          </a>
    ...
    auth.onAuthStateChanged((user) => {
        ...
        db.collection("orders")
            ...
            .onSnapshot(
                ...
                (error) => {
                    console.error("Farmer orders load failed:", error);
                    if (recentOrdersContainer) {
                        recentOrdersContainer.innerHTML = `
              <div class="p-5 text-sm text-red-600">Unable to load your recent buyer orders right now.</div>
            `;
                    }
                    if (oldOrdersContainer) {
                        oldOrdersContainer.innerHTML = `
              <div class="p-5 text-sm text-red-600">Unable to load your old buyer orders right now.</div>
            `;
                    }
                }
            );
